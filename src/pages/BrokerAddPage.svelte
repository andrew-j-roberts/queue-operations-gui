<script lang="ts">
  import { logs, queueManager, existsInQueueManager } from "../stores";
  import { addBrokerPage, close } from "../routes";
  ...
  function save() {
    // guards
    if (existsInQueueManager(localFormState.name)) {
      errorMessage =
        "Broker name already in use by Queue Manager, please use a unique name or edit the existing broker of that name.";
      ...
      const log: Log = {
        ...
        message: `Successfully saved new broker session named '${brokerSession.name}'.`,
      };
      logs.write(log);
      // close the page
      close(addBrokerPage());
...
<h1 class="mb-4 text-xl text-gray-800">Add new broker</h1>
